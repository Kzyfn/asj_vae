...
device = "cuda"
model = Rnn().to(device)
model.load_state_dict(torch.load("baseline1_pca8/baseline1_10.pth_10"))
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch % 5 == 0:
        torch.save(model.state_dict(), "baseline1_pca8/baseline1_{}.pth".format(epoch))
    np.save("baseline1_pca8/loss_list_baseline.npy", np.array(loss_list))
    np.save("baseline1_pca8/test_loss_list_baseline.npy", np.array(test_loss_list))
    np.save("baseline1_pca8/test_f0loss_list_baseline.npy", np.array(f0_list))
...
np.save("baseline1_pca8/loss_list_baseline.npy", np.array(loss_list))
np.save("baseline1_pca8/test_loss_list_baseline.npy", np.array(test_loss_list))
torch.save(model.state_dict(), "baseline1_pca8/baseline1.pth")
