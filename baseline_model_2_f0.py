...
def test(epoch):
    ...
    with torch.no_grad():
        for i, data, in enumerate(test_loader):
            ...
            f0_loss += F.mse_loss(recon_batch.view(-1), tmp[1]).item()
            del tmp
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch % 5 == 0:
        torch.save(
            model.state_dict(), "baseline1_f0/baseline1_{}.pth".format(epoch + 5)
        )
    np.save("baseline1_f0/loss_list_baseline.npy", np.array(loss_list))
    np.save("baseline1_f0/test_loss_list_baseline.npy", np.array(test_loss_list))
    np.save("baseline1_f0/test_f0loss_list_baseline.npy", np.array(f0_list))
...
np.save("baseline1_f0/loss_list_baseline.npy", np.array(loss_list))
np.save("baseline1_f0/test_loss_list_baseline.npy", np.array(test_loss_list))
torch.save(model.state_dict(), "baseline1_f0/baseline1.pth")
