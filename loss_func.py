...
def vae_loss(recon_x, x, mu, logvar):
    MSE = F.mse_loss(recon_x.view(-1), x.reshape(-1, ), reduction='sum')#F.binary_cross_entropy(recon_x.view(-1), x.view(-1, ), reduction='sum')
    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    #print(KLD)
    return MSE +  KLD

def vqvae_losss(recon_x, x, z, z_unquantized, beta=1):
